# Human 文档入口

## 作用

本目录保存给人阅读的项目正文文档，适合用来了解项目、执行现场操作、查看阶段结论和完成交接。

## 推荐阅读顺序

1. [overview/project_status.md](overview/project_status.md)
2. [overview/next_actions.md](overview/next_actions.md)
3. [overview/roadmap.md](overview/roadmap.md)
4. 当前阶段对应文档
5. 需要操作时再看 `operations/`

## 目录说明

### `overview/`

- 项目状态
- 下一步任务
- 整体基线

### `phases/`

- 各阶段的设计、报告、验收、SOP

### `operations/`

- 迁移、启动、问题索引

## 当前权威入口

- 当前状态：[overview/project_status.md](overview/project_status.md)
- 下一步任务：[overview/next_actions.md](overview/next_actions.md)
- 总体路线：[overview/roadmap.md](overview/roadmap.md)
- 当前架构分层：[overview/architecture_layers.md](overview/architecture_layers.md)
- 当前阶段路线图：[overview/phase_delivery_map.svg](overview/phase_delivery_map.svg)
- P4 阶段定义：[phases/p4/README.md](phases/p4/README.md)
- P4 完成清单：[phases/p4/completion_checklist.md](phases/p4/completion_checklist.md)
- P5 阶段定义：[phases/p5/README.md](phases/p5/README.md)
- P6 阶段定义：[phases/p6/README.md](phases/p6/README.md)
- P7 阶段定义：[phases/p7/README.md](phases/p7/README.md)
- P8 阶段定义：[phases/p8/README.md](phases/p8/README.md)
- 工位与模型对齐清单：[operations/hardware_alignment_checklist.md](operations/hardware_alignment_checklist.md)
- 仿真运行手册：[operations/simulation_runbook.md](operations/simulation_runbook.md)
- P2 验收：[phases/p2/acceptance_report.md](phases/p2/acceptance_report.md)
- P3-A 恢复 SOP：[phases/p3/recovery_sop.md](phases/p3/recovery_sop.md)
- P3-B 验证计划：[phases/p3/moveit_validation_plan.md](phases/p3/moveit_validation_plan.md)
- 运维问题索引：[operations/issue_index.md](operations/issue_index.md)
