...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Minimal left-arm FollowJointTrajectory smoke-test client."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.03,
        help="Small offset applied to left_joint1 from the current /joint_states sample.",
    )
    parser.add_argument(
        "--timeout",
        type=float,
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"left_arm_controller result: error_code={result.error_code}, "
            f"error_string={result.error_string}"
        )
        return 0 if result.error_code == 0 else 1
    except Exception as exc:
        print(f"send_left_arm_goal failed: {exc}", file=sys.stderr)
        return 1
    finally:
        node.destroy_node()
        rclpy.shutdown()
